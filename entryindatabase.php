<?php

	$name=$_POST["name"];
	#$location=$_POST["location"];
	$email=$_POST["email"];
    $mobile=$_POST["mobile"];
	$groupsize=$_POST["groupsize"];
	$date=$_POST["date"];

	$message=$_POST["message"];	
	
	$dbname= "enquiryform";

	if(!empty($name)||!empty($email)||!empty($mobile)||!empty($groupsize)||!empty($date)||!empty($message))
	{
		$host ="localhost";
		$dbUsername="root";
		$dbPassword="";
		$conn =new mysqli($host,$dbUsername,$dbPassword,$dbname);
		
					$stmt=$conn->prepare("INSERT into enquiryformcontent (name,email,mobile,date,message) values (?,?,?,?,?)");
					$stmt->bind_param("sssss",$name,$email,$mobile,$date,$message);
					$stmt->execute();
					#echo "New Account Succesfully Created";
	    		
				$stmt->close();
				$conn->close();
			
	}
	
?>


<!DOCTYPE html>
	<html lang="zxx" class="no-js">
	<head>
		<!-- Mobile Specific Meta -->
		<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<!-- Favicon-->
		<link rel="shortcut icon" href="img/fav.png">
		<!-- Author Meta -->
		<meta name="author" content="colorlib">
		<!-- Meta Description -->
		<meta name="description" content="">
		<!-- Meta Keyword -->
		<meta name="keywords" content="">
		<!-- meta character set -->
		<meta charset="UTF-8">
		<!-- Site Title -->
		<title>Industrial Visit</title>

		<link href="https://fonts.googleapis.com/css?family=Poppins:100,200,400,300,500,600,700" rel="stylesheet"> 
			<!--
			CSS
			============================================= -->
			<link rel="stylesheet" href="css/linearicons.css">
			<link rel="stylesheet" href="css/font-awesome.min.css">
			<link rel="stylesheet" href="css/bootstrap.css">
			<link rel="stylesheet" href="css/magnific-popup.css">
			<link rel="stylesheet" href="css/animate.min.css">
			<link rel="stylesheet" href="css/owl.carousel.css">
			<link rel="stylesheet" href="css/main.css">
			<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
	
		</head>



		
		<body>
			<!-- start banner Area -->
			<!-- start banner Area -->
			<section class="banner-area" id="home">
				<!-- Start Header Area -->
				<header class="header">
					<nav class="navbar navbar-expand-sm  navbar-light" style="color: rgb(255, 255, 255); background-color: rgb(255, 186, 51)">
						<div class="container">
							  <a class="navbar-brand" href="index.html">
							  	<img src="img/logo.png" width="100" ,height="50" alt="">
							  </a>
							  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
							    <span class="text-white lnr lnr-menu"></span>
							  </button>

							    <div class="collapse navbar-collapse justify-content-end align-items-center" id="navbarSupportedContent">
								    <ul class="navbar-nav">
										<li><a href="index.html">Home</a></li>
										<!--Dropdown-->
									    <li class="dropdown">
									      <a class="dropdown-toggle" href="#" id="navbardrop" data-toggle="dropdown">
									        Destination
									      </a>
									      <div class="dropdown-menu">
									        <a class="dropdown-item" href="India.html">India</a>
									        <a class="dropdown-item" href="International.html">International</a>
									      </div>
									    </li>

									    <li class="dropdown">
									      <a class="dropdown-toggle" href="#faq" id="navbardrop" data-toggle="dropdown">
									        Tour Focus
									      </a>
									      <div class="dropdown-menu">
									        <a class="dropdown-item" href="It.html">Information Technology</a>
									        <a class="dropdown-item" href="Electronics.html">Electronics</a>
									        <a class="dropdown-item" href="Architecture.html">Architecture</a>
									        <a class="dropdown-item" href="Hotel.html">Hotel Management</a>
									      </div>
									    </li>

									    <li class="dropdown">
									      <a class="dropdown-toggle" href="#" id="navbardrop" data-toggle="dropdown">
									        About
									      </a>
									      <div class="dropdown-menu">
									        <a class="dropdown-item" href="index.html#about">About Us</a>
									        <a class="dropdown-item" href="index.html#features">Features</a>
									        <a class="dropdown-item" href="index.html#Recently">Recently Added</a>
									        <a class="dropdown-item" href="index.html#faq">FAQ</a>
									        <a class="dropdown-item" href="index.html#contact">Contact Us</a>
									      </div>
									    </li>

										<li><a href="index.html#gallery">Gallery</a></li>
										<li><a href="blog.html">Blog</a></li>
										<!-- Dropdown -->
									    <li class="dropdown">
									      <a class="dropdown-toggle" href="#" id="navbardrop" data-toggle="dropdown">
									        Login
									      </a>
									      <div class="dropdown-menu">
									        <a class="dropdown-item" href="admin.html">Admin</a>
									        <a class="dropdown-item" href="user.html">User</a>
									      </div>
									    </li>
								    </ul>
									<div class="search-container">
	    								<form class="example" action="action_page.php" style="margin:auto;max-width:200px ">
	  										<input type="text" placeholder="Search.." name="search">
	  										<button type="submit"><i class="fa fa-search"></i></button>
										</form>
	  								</div>  
							    </div>
  							</div>						
						</div>
					</nav>
						
				</header>
				<!-- End Header Area -->	

					<br>
					<br>
					<br>

					<style type="text/css">
						body{background-image: url('img/slider3.jpg');
							background-size: cover;
							background-attachment: fixed;


							}

						.content{ background:white;
							margin: 100px auto;
								padding: 40px;
									width:50%;
							   }

							.forborder{
								border-color: grey; 
							}

					</style>
					<div class="content">
					<center><h1>Thank you for getting in touch with us:</h1><br><h3>We will email you the trip details</h3></center>
						<br>
					<div class="forborder">
						<center><p>We got your message and will get back to you as soon as possible.
						</p>

						<a href="readmore(agra).html" class="button"><bold>Go back</bold></a>
						</center>
					</div>
					
					</div>



	</section>
	</body>
</html>

